...
#                     if status_res.status_code == 200: return status_res.json()
#                     elif status_res.status_code >= 400: return None
...
#             # Pass pocket_residues through to the API call
...
#                 affinities = data.get("affinities", {})
#                 # Get the first ligand affinity data
#                 lig_key = next(iter(affinities)) if affinities else None
#                 lig_data = affinities[lig_key] if lig_key else {}
                
#                 prob = lig_data.get("affinity_probability_binary", [0])[0]
#                 pic50 = lig_data.get("affinity_pic50", [0])[0]
#                 ic50 = pow(10,-pic50)*1e6 #10(-pic50) * 1e6 # Convert pIC50 to micromolar (uM)

#                 results.append({
#                     'SMILES': smiles,
#                     'Affinity_Prob': float(lig_data.get("affinity_probability_binary", [0])[0]),
#                     'pIC50': float(lig_data.get("affinity_pic50", [0])[0]),
...
#                     print(f"Predicted IC50 (µM):     {ic50:.3f}")
#                                 })
...
class BoltzClient:
    ...
    async def compute_properties(self, smiles_list, sequence, pocket_residues=None):
        ...
        for smiles in smiles_list:
            ...
            if data:
                # 1. Extract structural confidence scores
                ptm = data.get("ptm_scores", [0])[0]
                iptm = data.get("iptm_scores", [0])[0]
                conf = data.get("confidence_scores", [0])[0]
                plddt = data.get("complex_plddt_scores", [0])[0]
                
                # 2. Extract ligand-specific affinity data
                affinities = data.get("affinities", {})
                lig_key = next(iter(affinities)) if affinities else None
                lig_data = affinities[lig_key] if lig_key else {}
                ...
                prob = lig_data.get("affinity_probability_binary", [0])[0]
                pic50 = lig_data.get("affinity_pic50", [0])[0]
                ic50_um = pow(10, -pic50) * 1e6  # Convert pIC50 to Micromolar (uM)
                ...
                # 3. Store results including RDKit descriptors
                results.append({
                    'SMILES': smiles,
                    'pTM': ptm,
                    'ipTM': iptm,
