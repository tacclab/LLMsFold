...
from src.services import NeoraLabViewerService, PubChemService
...
class MoleculeGenerator:
    ...
    async def _launch_best_result_viewer(self, output_dir: Path, final_hits: pd.DataFrame) -> str | None:
        """Uploads the top-ranked saved structure to NeoraLab and opens the viewer."""
        ...
        viewer_service = NeoraLabViewerService(
            self._settings.neoralab_api_base_url,
            viewer_url=self._settings.neoralab_viewer_url,
        )
        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        structure = structure_path.read_text(encoding="utf-8")
        access_token = await viewer_service.authenticate(
            client_id=client_id,
            client_secret=client_secret.get_secret_value(),
        )
        item_id = await viewer_service.upload_viewer_payload(
            access_token=access_token,
            candidate_id=best_candidate_id,
            metadata=metadata,
            structure_content=structure,
        )
        viewer_url = viewer_service.build_viewer_url(item_id)
        opened = webbrowser.open(viewer_url, new=2)
        if opened:
            logger.info("Opened NeoraLab Viewer for %s at %s", best_candidate_id, viewer_url)
        else:
            logger.warning(
                "Uploaded NeoraLab Viewer payload for %s but could not open a browser automatically: %s",
                best_candidate_id,
                viewer_url,
            )
        return viewer_url
    ...
    async def run(self, options: PipelineOptions) -> str | None:
        ...
        with make_progress_bar(
            ...
            viewer_launcher = await self._launch_best_result_viewer(options.output_dir, final_hits)
            step_progress.update(1)
